		...
		<tr>
			...
			<td background="images/topback.gif" width="130">
				<table width="100%" border="0" cellspacing="2" cellpadding="2">
					<tr>
						<td valign="middle" align="center"><a href="cart.asp"><img
								src="images/button_cart.gif" width="87" height="18" border="0"></a></td>
					</tr>
					<tr>
						<td valign="middle" align="center"><a
							href="checkorder_login.asp"><img src="images/button_ddcx.gif"
								width="87" height="18" border="0"></a></td>
					</tr>
					<tr>

						<td valign="middle" align="center"><a
							href="customer_register.asp"><img
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
						<tr>
							<td width="50%"><a href="index.jsp" class="red">首页</a> &gt;
								<a href="category.jsp?id=" class="red">${subCategory.category.name}</a>
								&gt; ${subCategory.name}</td>
				...
				<table width="100%" border="0" cellspacing="2" cellpadding="2">
					<tr>
						<td valign="middle" align="left">当前为第 ${pager.firstRow+1} 到
							${pager.lastRow+1} 条记录，共${pager.lastRow-pager.firstRow+1}条商品信息</td>
					</tr>
				</table>
				...
				<table width="99%" border="0" cellspacing="2" cellpadding="5">
					...
					<c:forEach items="${productList }" var="product">
						...
						<tr>
							...
							<td width="80%"><a href="product.asp?" class="productName">${product.name}</a>

								<!-- 判断是否显示折扣图标开始 --> <c:if test="${product.hotDeal}">
									<img src="images/hotprice.gif" width="24" height="24">
								</c:if> <!-- 判断是否显示折扣图标结束--> <a href="#"><img border=0
									src="images/addtocart.gif" width="30" height="18" alt="添加到购物车"></A></td>
						</tr>
						<tr>
							<td width="80%">
								<!-- 是否显示原价：条件是price not eq 0 --> <c:if
									test="${product.price != 0 }">            
            原价：<span class="hotPrice">${product.price}</span>元　现价： 
            </c:if> <!-- 是否显示原价 --> <!-- 是否显示价格：条件是price eq 0 --> <c:if
									test="${product.price eq 0 }">
            价格： 
           </c:if> <!-- 是否显示价格：条件是price eq 0 --> ${product.listPrice}元

							</td>
						</tr>
						<tr>
							<td width="80%">${product.description}</td>
						</tr>
				...
				<table width="100%" border="0" cellspacing="2" cellpadding="2">
					<tr>
						<td>当前为第 ${pager.firstRow+1} 到 ${pager.lastRow+1}
							条记录，共${pager.lastRow-pager.firstRow+1}条商品信息</td>
					</tr>
					<tr>
						<td align="center">
								...
								<tr>
									...
									<td width="15%" align="left">
										<!--是否显示前一页 -->
										 <c:if test="${pager.hasPrev }">
											<a href="subcategory.jsp?id=${subCategory.id}&currentPage=${pager.currentPage-1}"
												class="navi">前一页</a>
										</c:if> <!--是否显示前一页 -->
									</td>
									...
									<td width="15%" align="right">
										<!--是否显示最末页 --> <c:if test="${pager.pageCount gt 0 }">
											<a href="subcategory.jsp?id=${subCategory.id}&currentPage=${pager.pageCount}"
												class="navi">最末页</a>
										</c:if> <!--是否显示最末页 -->
									</td>
								</tr>
